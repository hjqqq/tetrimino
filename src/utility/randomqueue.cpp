...
void RandomQueueData::erase(int startIndex)
{
    allStart.erase(allStart.begin() + startIndex);
    int minStart = *std::min_element(allStart.begin(), allStart.end());
    std::cerr << minStart;
    if (minStart != 0){
	for (int i = 0; i != allStart.size(); ++i)
	    allStart[i] -= minStart;
	randomData.erase(randomData.begin(), randomData.begin() + minStart);
    }    
}
...
void RandomQueue::erase()
{
    randomQueueData->erase(startIndex);
}
